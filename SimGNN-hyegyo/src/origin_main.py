...
def main():
    ...
    SAVE_NAME=f'batch{BATCH_SIZE}_drop{args.dropout}_epoch{EPOCHS}_{EXPERIMENT_NAME}'
    ...
    #torch summary
    summary(teacher_model, {"features_1": torch.zeros((4,15)), "features_2":torch.zeros((16,15)) , "edge_index_1": torch.zeros((2,6), dtype=torch.int64) ,"edge_index_2": torch.zeros((2,30), dtype=torch.int64) })
    summary(student_model, {"features_1": torch.zeros((4,15)), "features_2":torch.zeros((16,15)) , "edge_index_1": torch.zeros((2,6), dtype=torch.int64) ,"edge_index_2": torch.zeros((2,30), dtype=torch.int64) })
    ...
    if args.load_path:
        ...
        writer=SummaryWriter(log_dir=f"./runs/batch{BATCH_SIZE}_drop{args.dropout}_epoch{EPOCHS}_{EXPERIMENT_NAME}")
        # model fit(train)
